...
def home_view(request):
    if request.method == 'GET':
        ...
        for j in range(len(recommends)):  # 生成推荐数据
            d = YkLesson.objects.filter(yk_recommend_id=recommends[j][0]).values_list('id', 'yk_lesson_name',
                                                                                      'yk_lesson_img',
                                                                                      'yk_lesson_price',
                                                                                      'yk_buy_amount',
                                                                                      'yk_lesson_price_type',
                                                                                      'yk_lesson_describe')[:14]
        ...
        more_lessons = lesson.exclude(yk_recommend_id__in=s).values_list(
            'id', 'yk_lesson_name', 'yk_lesson_img', 'yk_lesson_price', 'yk_buy_amount', 'yk_lesson_price_type',
            'yk_lesson_describe')[:20]
        more_lesson = {}
