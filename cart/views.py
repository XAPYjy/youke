...
def get_cart(request):
    ...
    for i in list_cart_id:
        carts1 = YkLesson.objects.filter(id=i).values('id','yk_lesson_name','yk_course_chapter','yk_lesson_price','yk_lesson_img')[0]
...
def after_buy(request):
    ...
    for i in nums:
        print(i)
        global result
        goodsid = YkOrder.objects.filter(id=i,yk_user_id=user_id).values('yk_goods_id')[0]['yk_goods_id']
        bag = Bags.objects.filter(yk_lesson_id=goodsid,yk_user_id=user_id)
        now = time.localtime()
